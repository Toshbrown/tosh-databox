...
func (arb *ArbiterClient) GetRootDataSourceCatalogue() (types.HypercatRoot, error) {

	cat, status := arb.makeArbiterRequest("GET", "/cat", arb.arbiterURL, "/cat", "GET")
	if status != 200 {
		err := errors.New(strconv.Itoa(status) + ": " + " GET " + " /cat Failed")
		return types.HypercatRoot{}, err
	}

	rootCat := types.HypercatRoot{}
...
func (arb *ArbiterClient) UpdateArbiter(name string, tokenString string, databoxType types.DataboxType) error {
