...
func Setup() {
	...
	hostname = os.Getenv("DATABOX_LOCAL_NAME")
	arbiterURL = os.Getenv("DATABOX_ARBITER_ENDPOINT")
...
func dataSourceMetadataToHypercat(metadata types.DataSourceMetadata, endPoint string) ([]byte, error) {
	...
	cat.ItemMetadata = append(cat.ItemMetadata, types.RelValPair{Rel: "urn:X-databox:rels:hasType", Val: metadata.DataSourceType})
	cat.ItemMetadata = append(cat.ItemMetadata, types.RelValPair{Rel: "urn:X-databox:rels:hasDatasourceid", Val: metadata.DataSourceID})
	cat.ItemMetadata = append(cat.ItemMetadata, types.RelValPair{Rel: "urn:X-databox:rels:hasStoreType", Val: metadata.StoreType})

	if metadata.IsActuator {
		cat.ItemMetadata = append(cat.ItemMetadata, types.RelValPairBool{Rel: "urn:X-databox:rels:isActuator", Val: true})
	}

	if metadata.Location != "" {
		cat.ItemMetadata = append(cat.ItemMetadata, types.RelValPair{Rel: "urn:X-databox:rels:hasLocation", Val: metadata.Location})
	}

	if metadata.Unit != "" {
		cat.ItemMetadata = append(cat.ItemMetadata, types.RelValPair{Rel: "urn:X-databox:rels:hasUnit", Val: metadata.Unit})
	}
